...
class CrawlWebsiteCategories(scrapy.Spider):  # [ -- Requests Sent by order and Receive Responses (to crawl) not the same order -- ] -=-=->> -=-=->> -=-=->> -=-=->> -=-=->> -=-=->>>> (- - [DW] - -)
    name = "crawl_website_categories"  # in one website and more than categories
    start_urls = ['https://nasainarabic.net/main/categories/view/mearth',
                  'https://nasainarabic.net/main/categories/view/outer-space',
                  'https://nasainarabic.net/main/categories/view/technology',
                  'https://nasainarabic.net/main/categories/view/memory']  # ______more than ((website))______

    def start_requests(self):
        for start_url in self.start_urls:  # ______more than ((website))______
            yield scrapy.Request(url=start_url, callback=self.parse, meta={'category': start_url.split('/view/')[1]})
    ...
    def __init__(self, **kwargs):
        ...
        self.counter = -1
        self.depth = self.time_converter('2020-07-26 17:34:09')
    ...
    def parse(self, response, **kwargs):  # scroll pages  ( response : is url to first page )
        ...
        if next_page:
            ...
            if self.move_to_next_page and self.category_type == response.meta['category']:  # (False) if cross the Depth in category
                yield scrapy.Request(url=next_url_page, callback=self.parse, meta={'category': response.meta['category']})  # To Open (URL)  [-Do'nt put (Yield'request') in (loop'for')-] && [-((DW))- open page By page (all pages have the same format to move to next page) -((DW))-]
